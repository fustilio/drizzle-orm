...
import { alias, bigint, boolean, integer, jsonb, pgEnum, pgSchema, pgTable, pgTableCreator, QueryBuilder, serial, text, timestamp, varchar } from 'drizzle-orm/pg-core';
import { getValueFromDataApi } from '~/aws-data-api/common';
...
test.before(async (t) => {
	...
	`;
	await ctx.db.execute(sql`CREATE SCHEMA IF NOT EXISTS "drizzle"`);
	await ctx.db.execute(migrationTableCreate);

});

const CardType = pgEnum('CardType', ['VOCABULARY', 'GRAMMAR']);

const Cards = pgTable('Cards', {
	id: serial('id').primaryKey(),
	type: varchar('type', { enum: CardType.enumValues }).notNull().default('VOCABULARY'),
	keyphrase: varchar('keyphrase').notNull(),
...
test.beforeEach(async (t) => {
	...
	// const dbMigrations = await ctx.db.execute(
	// 	sql.raw(`select id, hash, created_at from "drizzle"."__drizzle_migrations" order by created_at desc `)
	// );
	// console.log(dbMigrations)

	// const db2 = await ctx.db.session.execute(
	// 	sql.raw(`select id, hash, created_at from "drizzle"."__drizzle_migrations" order by created_at desc `)
	// )
	...
	// const db4 = await ctx.db.session.all(sql.raw(`select id, hash, created_at from "drizzle"."__drizzle_migrations" order by created_at desc `))

	// console.log("db4", db4);

	// const db5 = await ctx.db.select().from(drizzleMigrations)
	...
	// 	const query = qb.select().from(drizzleMigrations)

	// const db6 = await ctx.db.session.execute(query.getSQL())
	// console.log("db6", db6)


	// const db7 = await ctx.db.transaction(async (tx) => {

	// })
	// console.log("db4", db4.map((query) => query.map((val) => getValueFromDataApi(val))));
	// const write = await ctx.db.execute(
	// 	t.session.execute(
	...
	// await ctx.db.execute(sql`CREATE schema IF NOT EXISTS public`);
	...
	// const dbMigrations = await ctx.db.execute<{ id: number; hash: string; created_at: string }>(
	// 	sql`select id, hash, created_at from "drizzle"."__drizzle_migrations" order by created_at desc limit 1`,
	// );

	console.log("create table users performed")
});

const sleep = (ms: number) => new Promise((r) => setTimeout(r, ms));

test.serial('no-op', async (t) => {
	// const ctx = t.context;
	// const dbSelect = await ctx.db.select().from(drizzleMigrations)
